  ...
  setup(): void {
    ...
    const fragmentSrc = `#version 300 es
      ...
      void main() {
        ...
        for(float i=1.;i<8.;i++) {
          uv+=.1*cos(i*vec2(.1,.8)+time*.5);
          float d=length(uv);
          col+=.002/d*cos(sin(i)*vec3(.4,.7,1.));
          col=mix(col,vec3(.05,.1,.25)*noise(i+uv),d);
        }
...
const useShaderBackground = (isDarkMode: boolean) => {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const rendererRef = useRef<WebGLRenderer | null>(null)
  const rafRef = useRef<number | null>(null)
  ...
  useEffect(() => {
    ...
    // Ativar/desativar WebGL com base no tema
    const renderer = rendererRef.current
    if (isDarkMode) {
...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative w-full h-screen overflow-hidden bg-gray-50 dark:bg-black">
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center text-gray-900 dark:text-white px-6">
        <div className="text-center space-y-10 max-w-6xl mx-auto">
          <div className="space-y-4">
            ...
            <h1 className="text-6xl md:text-8xl lg:text-9xl font-extrabold">
              <span className="bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 dark:from-cyan-400 dark:via-blue-500 dark:to-indigo-500 bg-clip-text text-transparent">Tecnologia</span>
            </h1>
          </div>
          <div className="w-40 h-1 bg-gradient-to-r from-transparent via-cyan-600 dark:via-cyan-500 to-transparent mx-auto" />
          <div className="max-w-4xl mx-auto">
            <p className="text-xl md:text-2xl lg:text-3xl text-gray-700 dark:text-cyan-50/90 font-light leading-relaxed">
              A LT Cloud transforma ideias em soluções digitais de alto desempenho para empresas que buscam excelência tecnológica.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-6 justify-center mt-14">
            <a href="#services" className="px-10 py-5 bg-gradient-to-r from-cyan-600 to-blue-600 dark:from-cyan-500 dark:to-blue-600 text-white rounded-xl font-semibold text-xl transition-transform duration-300 hover:scale-105 shadow-xl shadow-cyan-500/40">
              Nossos Serviços
            </a>
            <a href="#contact" className="px-10 py-5 bg-cyan-600/10 dark:bg-cyan-500/10 border border-cyan-600/30 dark:border-cyan-300/30 text-cyan-700 dark:text-cyan-100 rounded-xl font-semibold text-xl transition-transform duration-300 hover:scale-105 backdrop-blur-md">
              Entre em Contato
            </a>
          </div>
        </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2">
        <div className="w-6 h-10 border-2 border-cyan-600/50 dark:border-cyan-400/50 rounded-full flex items-start justify-center p-2 animate-bounce">
          <div className="w-1 h-2 bg-cyan-600 dark:bg-cyan-400 rounded-full"></div>
        </div>
      </div>
    </section>
  )
